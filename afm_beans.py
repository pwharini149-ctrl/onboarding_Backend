...
# ---------- CONFIG ----------
MAPPING_FILE = "AFM/Templates/gl_afm_mapping.json"
DETAILS_FILE = "AFM/Templates/gl_afm_beans.json"
BASE_FILE = "AFM/Templates/gl_afm_base.xml"
...
def afm_outer_beans_file(template_path, modifications=None):
    """
...
def afm_gl_beans_file(data):
    try:
        data = request.get_json(force=True)
        if not data or "search_keys" not in data:
            return jsonify({"error": "Missing 'search_keys'"}), 400

        search_keys = [k.strip() for k in data["search_keys"] if k.strip()]
        if not search_keys:
        ...
        search_to_config = {m["search_key"]: m["config_name"] for m in mapping_data}
